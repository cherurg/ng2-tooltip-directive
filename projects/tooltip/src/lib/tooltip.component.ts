...
export class TooltipComponent implements OnInit {
  ...
  setPosition(): void {
    ...
    const elementHeight = isSvg
      ? this.element.getBBox().height
      : this.element.offsetHeight
    const elementWidth = isSvg
    ...
    if (this.placement === 'top' || this.placement === 'bottom') {
      this.hostStyleLeft =
        this.elementPosition.left + elementWidth / 2 - tooltipWidth / 2 + 'px'
    }

    if (this.placement === 'left') {
      this.hostStyleLeft =
        this.elementPosition.left - tooltipWidth - this.tooltipOffset + 'px'
    }

    if (this.placement === 'right') {
      this.hostStyleLeft =
        this.elementPosition.left + elementWidth + this.tooltipOffset + 'px'
    }
